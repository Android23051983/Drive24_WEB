      ...
      <ul class="navbar-nav mb-2 mb-lg-0 text-right">
        ...
        if(empty($_SESSION["user"]) && empty($_SESSION["admin"])){ ?>
            ...
            if($_SESSION['user']->role == 12){
              ?>
               <li><a class="dropdown-item border border-dark bg-info text-dark rounded-3" href="index.php?page=9">Личный кабинет</a></li>
              <?
            }
             if($_SESSION['user']->role == 123){
              ?>
               <li><a class="dropdown-item border border-dark bg-info text-dark rounded-3" href="index.php?page=10">Личный кабинет</a></li>
              <?
            }
          if(!empty($_SESSION["user"]) || !empty($_SESSION["admin"])) {
            ?>
            <li><a class="dropdown-item border border-dark bg-info text-dark rounded-3" href="pages/logout.php">Выход</a></li>
            <? } ?> 
          </ul>
        </li>
      </ul>
