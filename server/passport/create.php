...
if ($passport->create()) {
    ...
    echo json_encode(array("message" => "Паспорт паспорта был создан."), JSON_UNESCAPED_UNICODE);
} else {
    ...
    echo json_encode(array("message" => "Невозможно создать паспорт пользователя."), JSON_UNESCAPED_UNICODE);
}
} else {
        http_response_code(503);
